...
Versione con xmin, xmax FISSI (come nell'esempio originale del libro).

A differenza di es1_ml.py (che stimava xmin, xmax dai dati), qui si assume
che l'accettanza dello strumento sia nota a priori: Xmin=-0.95, Xmax=0.95,
esattamente come nel testo. E' la scelta metodologicamente piu' corretta:
gli estremi del nostro campione (-0.9425/0.9422) sono cosi' vicini a questi
valori da suggerire che il dataset sia stato generato con lo stesso taglio
strumentale usato nell'esempio del libro. Usare il min/max campionario come
se fosse il limite noto dello strumento e' concettualmente diverso (e leggermente
distorsivo): il libro assume che [Xmin,Xmax] sia una caratteristica nota del
rivelatore, non qualcosa da stimare dal campione stesso.
...
xmin, xmax = -0.95, 0.95  # accettanza nota dello strumento (come nel libro)
n = len(data)
print(f"n = {n}, xmin = {xmin} (fisso), xmax = {xmax} (fisso)")
print(f"[per confronto: estremi campionari osservati {data.min():.4f} / {data.max():.4f}]")
